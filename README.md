...
0. Тесты проводить в отдельном каталоге в YC. При его создании (например, каталог `stage`) запомнить его id,  
   т.к. он потребуется в дальнейшем при запуске тестов.

1. [Установить](https://golang.org/doc/install) последнюю версию Go локально  
...
5. Перед запуском выполнить инициализацию модуля Go:
  ...
  go mod init test  
  ```
6. Все новые зависимости в файле `end2end_test.go` блоке `import` необходимо подгружать командой:
  ```bash
  go mod vendor
  ```
7. Запуск тестов:
...
9. Пример команды запуска теста с использованием приватного ssh-ключа целевого сервера:
  ```bash
